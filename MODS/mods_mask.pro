...
;   22-Jan-2019  Changed test *LBT-SX* to *LBT-* to accommodate MODS1 (SX)
;                and MODS2 (DX) focal stations [rwp/osu]
;-  
;------------------------------------------------------------------------------
; Add the vector XPOS_ADD (and XERR_ADD) to the array XPOS (XERR)
; as the i-th row of that array.
pro mods_mask_add1, i, xpos, xerr, xpos_add, xerr_add
...
;------------------------------------------------------------------------------
pro mods_mask, filename, outfile, biasfile = biasfile $
		   ...
		   , YRMASK = yrmask,INTERACTIVE_SLITS=INTERACTIVE_SLITS,AUTOTUNE_SLITS=AUTOTUNE_SLITS
...
;we have already processed with pixelflats
...
image = xmrdfits(filename, 0, hdr, /fscale,/silent)
...
telid      = strtrim(sxpar(hdr[*,0], 'TELID'), 2)
obsmask    = strcompress(sxpar(hdr[*,0], 'MASKNAME'), /rem)
;mms        = strcompress(sxpar(hdr[*,0], 'MASKINFO')+'.mms', /rem)
grat       = strcompress(sxpar(hdr[*,0], 'GRATNAME'), /rem)      

splog,'TELESCOPE: ',telescope
splog,'DETECTOR: ',detector
splog,'MASK: ',obsmask
...
; 5x60 slits
IF strmatch(telescope, '*LBT-*') AND strmatch(obsmask, '*LS5*') THEN BEGIN
  mms = GETENV('XIDL_DIR') + '/Spec/Longslit/pro/LBT/MODS/Calib_Lib/facility_masks/' + strlowcase(obsmask) + '.mms'
  verify = file_test(mms)
  if not verify then begin
	splog,string(mms)+" not located. Aborting."
	stop
  endif
  tset_slits = mods_slitmask(mms, nx, ny, CHANNEL = instrument,GRATNAME=grat,filename = filename,$
		INTERACTIVE_SLITS=INTERACTIVE_SLITS,AUTOTUNE_SLITS=AUTOTUNE_SLITS) 
ENDIF ELSE IF strmatch(telescope, '*LBT-*') AND strmatch(obsmask, '*LS60*') THEN BEGIN
  mms=GETENV('XIDL_DIR') + '/Spec/Longslit/pro/LBT/MODS/Calib_Lib/facility_masks/' + strlowcase(obsmask) + '.mms'
  verify = file_test(mms)
  if not verify then begin
        splog,string(mms)+" not located. Aborting."
  ...
  tset_slits = mods_slitmask(mms, nx, ny, CHANNEL = instrument,GRATNAME=grat,filename = filename,$
                INTERACTIVE_SLITS=INTERACTIVE_SLITS,AUTOTUNE_SLITS=AUTOTUNE_SLITS)
ENDIF ELSE IF strmatch(telescope, '*LBT-*') THEN BEGIN
  mms = obsmask + '.mms'
  verify_maskname = file_test(mms)
  if not verify_maskname then begin
        splog,string(mms)+" not located, MOS mms masks must be supplied in the working directory"
	stop
  endif
  splog,'Located '+string(mms)
  tset_slits = mods_slitmask(mms, nx, ny, CHANNEL = instrument,GRATNAME=grat,filename = filename,$
		INTERACTIVE_SLITS=INTERACTIVE_SLITS,AUTOTUNE_SLITS=AUTOTUNE_SLITS) 
ENDIF ELSE BEGIN
...
;; Fit these initial traces by a polynomial function
   ...
   FOR kk = 0L, nslit-1L DO BEGIN
      ...
      ;; KHRR added keyword YRMASK 
      if keyword_set(YRMASK) then begin
         max_now = max(flat_now[yrmask[0]:yrmask[1]], kmax)
         ycen[kk] = float(kmax) + float(yrmask[0])
      endif else begin
...
ENDELSE
   ...
   slitmask = mods_slits2mask(tset_slits, nslit = nslit)
   ;; Write output file
