...
pro mods_flatfield_specillum, x, y, image, spec_set, illum_set $
                              , invvar = invvar, slitwidth = slitwidth $
                              , modelfit = modelfit, ybkpt = ybkpt $
                              , npoly = npoly, TOL_EDG = TOL_EDG, CHK = CHK $
                              , pixfit = pixfit, SLITSAMP = SLITSAMP, DEBUG=IGOOD
...
;     Now try a slit-illumination fit
     ...
     illumquick1 = djs_median(filtimg[isamp], boundary = 'reflect') ;removed - , width = samp_width
     ...
     IF NOT KEYWORD_SET(PIXFIT) THEN RETURN
      ...
      inmask = (invvar[xs] GT 0.0) AND $
        abs((image[xs]-specmodel1*illummodel1[xs])*sqrt(invvar[xs])) LT 15.0
...
;        (y[xs] LE (1.0D - TOL_EDG/SLITWIDTH))
      ...
      scatillum_set = bspline_longslit(x[xs], image[xs] $
                                       ...
                                       , inmask = inmask $
                                       , nord = 4 $
                                       , upper = 9, lower = 9);, DEBUG=[IGOOD,xs])
;      , upper = 7, lower = 7)
