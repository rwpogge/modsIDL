...
;
; CALLING SEQUENCE:
;   xshift=mods_xcorr_slits(image1, tset_slits, maxlag=, /silent,/shift)
;
; INPUTS:
...
;   2013      rewriten to be adapted to MODS data by K. Croxall
;-
;------------------------------------------------------------------------------
FUNCTION mods_xcorr_slits, image1, tset_slits, maxlag = maxlag $
                           , silent = silent, shift = shift $
                           , lags = lags, corr = corr
                           

if (size(tset_slits[0].XX1, /n_dimen) EQ 1) then nslit = 1 $
ELSE nslit = (size(tset_slits[0].XX1, /dimens))[1]
...
maskim = mods_slits2mask(xcorr_slits, nslit = nslit1)
image2 = (maskim NE 0)
...
corr = c2_correlate(image2, image1, lags)
