...
;-
;------------------------------------------------------------------------------
FUNCTION MODS_REIDENTIFY, arc_obj, lines, wstruct $
...
;2000 worked for blue... but messed up the red
MXSHIFT = 900    ;made quite large for MODS multislit... 1000 may be a better value
...
if arc_inter NE 2 then begin
    ...
    IF nbest GT 0 THEN BEGIN
        ...
        splog, 'long_reidentify: Using ', best_ind[jmax], ' from the archive.'
    ENDIF ELSE BEGIN
...
ffitdim=size(*fit_ref.ffit,/dimensions)
IF fit_ref.nord NE ffitdim[0] THEN BEGIN
	 *fit_ref.ffit=transpose(*fit_ref.ffit)
;	splog,'transposed the ffit'
ENDIF
