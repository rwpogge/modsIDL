...
PRO mods_reduce_work, filename, scifile, ISLIT = ISLIT $
...
sciimg = xmrdfits(filename, 0, hdr, /fscale,/silent)
...
ximg = mods_slits2x(tset_slits, edgmask = edgmask, nslit = nslit)
slitmask = mods_slits2mask(tset_slits) * (ximg GT 0. AND ximg LT 1.)
  if not keyword_set(wavefile) then begin
             instrument = strcompress(sxpar(scihdr[*, 0], 'INSTRUME'), /rem)
	     mask = strcompress(sxpar(scihdr[*, 0], 'MASKNAME'), /rem)
 	     if strmid(mask,0,2) eq 'LS' then mask = 'LS'

	     CASE instrument OF
		'MODS1B': wavefile = 'wave-comp_m1b_'+mask+'.fits'
		'MODS1R': wavefile = 'wave-comp_m1r_'+mask+'.fits'
		'MODS2B': wavefile = 'wave-comp_m2b_'+mask+'.fits'
		'MODS2R': wavefile = 'wave-comp_m2r_'+mask+'.fits'
	     ENDCASE
	     verify = file_test(wavefile)
             splog,'You have not given me a wavelength calibration file in the parameter file!'
             if verify then splog,'Continue under the assumption the correct wavefile is '+wavefile
             if not verify then begin 
		print,'----------------------------------------------------------'
		splog,wavefile+' does not exist'
		print,"to coninue please declare: wavefile='filename'"
                print,"and then enter .continue"
                print,'----------------------------------------------------------'
		stop
	     endif
  endif

IF strcmp(gratname, 'P450L') OR (strcmp(gratname, 'P700L')) THEN BEGIN
...
if keyword_set(ISLIT) then begin
    ...
    if nct GT 0 then begin
        print, 'mods_reduce_work:  Will save other slits as previously reduced.'
...
;hard code skipping illumination correction within a slit as it is negligible
; if one worries they can check the illumination flat produced.
slit_illum = 0.0*sciimg +1.0
...
IF KEYWORD_SET(SKYTRACE) THEN BEGIN
    ...
    ;;trcchk = 1
    pixset = mods_wavepix(sciimg, tset_slits, fwhm = fwhmset.MEDIAN $
...
;update the header
  header_line = 'mods_reduce beta version 2014-08-21'
  sxaddpar,scihdr,'HISTORY',header_line
  sxaddpar,scihdr,'SlitFile',slitfile
  sxaddpar,scihdr,'wavefile',wavefile
  header_line = 'mods_reduce: Image Transposed'
  sxaddpar,scihdr,'HISTORY',header_line
;  header_line = 'mods_reduce: Extension 0 - Image'
;  sxaddpar,scihdr,'HISTORY',header_line
;  header_line = 'mods_reduce: Extension 1 - Variance'
;  sxaddpar,scihdr,'HISTORY',header_line

; Peel off for extended sources where we want to manually 
; influence the sky subtraction and extraction
if NOT KEYWORD_SET(EXT1D) THEN BEGIN
	; Write output file
	splog, 'Writing FITS file ', scifile
	mwrfits, float(sciimg), scifile, scihdr[*, 0], /create
	;for ap=0L, nslit-1L do begin
	;        mwrfits, *rect_arr(ap), scifile
	;endfor
	mwrfits, float(sciivar), scifile
...
objstruct = mods_objfind(sciimg-skyimage, tset_slits = tset_slits $
...
orig_sky = skyimage
;; expand slit edges
mods2xy, tset_slits[0], yy1, xx1
mods2xy, tset_slits[1], yy2, xx2
modelivart = sciivar
...
;bin not being read for MODS hard code to 1x1 for now
bin=[1L,1L]
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;add another skysub layer
subimg=sciimg-skyimage
...
mwrfits, float(outmask)*float(slitmask GT 0), scifile
mwrfits, final_struct, scifile
mwrfits, float(subimg)*float(slitmask GT 0), scifile
